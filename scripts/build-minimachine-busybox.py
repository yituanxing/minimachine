...
def main() -> int:
    p = argparse.ArgumentParser(
        description="Build a real BusyBox whole-LTO MiniMachine userspace image."
    )
    p.add_argument("input", type=Path, help="whole-program BusyBox LLVM .ll")
    p.add_argument("output", type=Path, help="output bFLT/MMP3 image")
    p.add_argument("--entry", default="ash_main")
    p.add_argument(
        "--entry-args",
    ...
    if decoded.entry != program.entry:
        raise RuntimeError("BusyBox user image round-trip changed entry")
    if len(decoded.functions) != len(program.functions):
        raise RuntimeError("BusyBox user image round-trip changed function count")
    ...
    print(
        "BUSYBOX_BUNDLE "
        f"entry={program.entry} functions={report['functions']} "
    ...
    for name in source_image.external_functions:
        print(f"BUSYBOX_EXTERNAL_FUNCTION {name}", flush=True)
    for name in source_image.external_data:
        print(f"BUSYBOX_EXTERNAL_DATA {name}", flush=True)
